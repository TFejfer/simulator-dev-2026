...
final class DescriptionRepository
{
    ...
    public function read(int $accessId, int $teamNo, int $outlineId, int $exerciseNo, int $themeId, int $scenarioId): array
    {
        ...
        return $row ?: ['short_description' => '', 'long_description' => '', 'work_notes' => ''];
    }

    public function upsert(
        int $accessId, int $teamNo, int $outlineId, int $exerciseNo,
        int $themeId, int $scenarioId,
        string $short, string $long, string $notes,
        string $actorToken
    ): void {
        $stmt = $this->db->prepare("
            INSERT INTO problem_form_description
              (access_id, team_no, outline_id, exercise_no, theme_id, scenario_id,
               short_description, long_description, work_notes, actor_token, updated_at)
            VALUES
              (:access_id, :team_no, :outline_id, :exercise_no, :theme_id, :scenario_id,
               :short, :long, :notes, :actor_token, CURRENT_TIMESTAMP)
