...
(() => {
	...
	const DeviationExpl = (id, fallback = '') => tMap('deviation_explanation', id, fallback);
	...
	const buildSymptomText = (row, themeId) => {
		...
		const clarify = String(row.clarify_text || '').replace(/\\/g, '');
	...
	const buildDeviationSelect = () => {
		...
		return `
			<select name="deviation" class="form-control" id="deviation" required>
				...
				${entries.map(([id, text]) => `<option value="${id}">${esc(text)}</option>`).join('')}
			</select>
		`;
	};
	...
	const buildFunctionSelect = (themeId) => {
		...
		return `
			<select name="function" class="form-control" id="function" required>
				<option value="0">-- ${Problem(74, 'Select function')} --</option>
				${rows.map((r) => `<option value="${r.function_id}">${esc(r.text_value || '')}</option>`).join('')}
			</select>
		`;
	};
	...
	const modalFormHtml = (id, themeId, lockSelects) => {
		...
		const explanation = [1, 2, 3, 4].map((i) => `
			<div>
				<span class="deviation-term">${esc(deviationText(i))}</span> :
				<span class="deviation-definition">${esc(DeviationExpl(i))}.</span>
			</div>
		`).join('');
			...
			? rows.map((row) => {
				...
				return `
					<fieldset class="symptom-element ${canEdit ? 'case-field clickable symptom-update-modal' : 'case-field-readonly'}" data-id="${id}">
						...
						<div class="symptom-text ${canEdit ? 'case-item-clickable' : 'case-item'}" data-id="${id}" contenteditable="false">${esc(text)}</div>
					</fieldset>
	...
	const bind = ({ store, scope }) => {
		...
		$(document).on('click', '.symptom-update-modal', function () {
			...
			$('.specification').text(String(row.clarify_text || ''));
			$('.delete-symptom').attr('data-prio', String(row.is_priority || 0));
