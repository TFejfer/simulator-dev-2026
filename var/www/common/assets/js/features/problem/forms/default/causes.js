...
const CausesForm = (() => {
			...
			<div class="grid-cause-add">
				<div>
					<select name="causeCi" class="form-control" id="causeCi" required>
						<option value="00O">-- ${H.esc(Problem(21, 'Select item'))} --</option>
						${options}
					</select>
				</div>

				<fieldset class="case-field">
					<legend class="case-label">${H.esc(Problem(58, 'Deviation'))}</legend>
		...
		return `
			<div class="grid-cause">
				<fieldset class="cause-text case-field-readonly">
					<legend class="case-label">${H.esc(Problem(29, 'Possible cause'))}</legend>
				...
				<fieldset class="cause-liho ${readOnlyLikelihood ? 'case-field-readonly' : 'case-field'}">
					<legend class="case-label">${H.esc(Problem(17, 'Likelihood'))}</legend>
				...
				<fieldset class="cause-evid case-field">
					<legend class="case-label">${H.esc(Problem(26, 'Evidence'))}</legend>
					<div id="edit_cause_evidence" class="case-edit" contenteditable="true">${H.esc(c.evidence_text ?? '')}</div>
				</fieldset>

				<div class="cause-proof cause-proven ${parseInt(c.is_proven || 0, 10) ? 'cause-selected' : ''}">${H.esc(Problem(23, 'Proven'))}</div>
				<div class="cause-proof cause-disproven ${parseInt(c.is_disproven || 0, 10) ? 'cause-selected' : ''}">${H.esc(Problem(24, 'Disproven'))}</div>
			</div>
		`;
	};
	...
	const render = (store, view) => {
		...
		if (causes.length) {
			...
			listHtml = ordered.map((c) => {
				...
				const label = isProven ? Problem(25, 'True cause') : Problem(29, 'Possible cause');
		...
		const addBtn = (canEdit && causes.length < 50)
			? `<span class="add-cause clickable link-text">${H.esc(Problem(36, 'Add possible cause'))}</span>`
			: '';

		$(rootId).html(`
			<details class="form-method">
				<summary class="form-step-header">3. ${H.esc(Problem(4, 'Causes'))}</summary>
				${renderRestoreMethod()}
			</details>
	...
	const bind = (ctx, view) => {
		...
		$(document).on(`click${EVENT_NS}`, '.add-cause', () => {
			$('#simulator_modal_title').html(H.esc(Problem(29, 'Cause')));
